...
import br.com.maury.clinicaServlet.atendimento.dao.ContatoPessoaFisicaDAO;
import br.com.maury.clinicaServlet.atendimento.dao.EnderecoDAO;
import br.com.maury.clinicaServlet.atendimento.dao.FuncionarioDAO;
import br.com.maury.clinicaServlet.atendimento.dao.PessoaFisicaDAO;
...
import br.com.maury.clinicaServlet.sistema.dao.UsuarioDAO;
import br.com.maury.clinicaServlet.sistema.modelo.Modulo;
import br.com.maury.clinicaServlet.sistema.modelo.Usuario;
import br.com.maury.clinicaServlet.web.util.CaniveteSuico;
import br.com.maury.clinicaServlet.web.util.CarregadorDeAtributoRequest;

@SuppressWarnings("serial")
@WebServlet(urlPatterns = "/novoProfissional")
public class NovoProfissional extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		if (FiltroDeAuditoria.acessoValido(req, resp)) {
			...
			RequestDispatcher dispatcher = req
					.getRequestDispatcher("WEB-INF/paginas/atendimento/cadastrarNovoProfissional.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String crp = req.getParameter("crp");
		...
		try (Connection con = ConnectionPool.getConnection()) {
			// Desativa o autocommit. � necess�rio realizar o commit ou rollback
			...
			if (contatoDAO.gravar(contato)) {
				...
				if (enderecoDAO.gravar(endereco)) {
					...
					if (usuarioDAO.gravar(usuario)) {
						...
						Profissional profissional = new Profissional(nome, sobrenome, rg, cpf, dataNascimento,
								observacao, endereco, contato, sexo, nivelEscolaridade, idade, departamento, cargo,
								dataAdmissao, true, usuario, unidades, crp);

						PessoaFisicaDAO pfDAO = new PessoaFisicaDAO(con);

						if (pfDAO.gravar(profissional)) {
							FuncionarioDAO funcionarioDAO = new FuncionarioDAO(con);
							if (funcionarioDAO.gravar(profissional)) {
								ProfissionalDAO profissionalDAO = new ProfissionalDAO(con);
								if (profissionalDAO.gravar(profissional)) {
									con.commit();
								} else {
									con.rollback();
								}
